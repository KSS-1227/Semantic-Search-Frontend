...
export const searchApi = async (
  ...
  try {
    ...
    return response.results.map((result: any) => ({
      ...
      explainability: `Similarity score: ${(result.similarity * 100).toFixed(
        1
      )}%`,
    }));
  } catch (error) {
...
export const getAnalyticsData = async (): Promise<AnalyticsData> => {
  try {
    ...
    const topQueries =
      response.topQueries?.slice(0, 5).map((item: any) => ({
        name: item.query,
        value: item.count,
      })) || [];
    ...
    const searchSuccessRate = [
      {
        name: "Successful",
        value: Math.round(response.successRate?.successRate || 85),
      },
      {
        name: "Unsuccessful",
        value: Math.round(100 - (response.successRate?.successRate || 85)),
      },
    ];
...
const getMockSearchResults = (
  ...
  const mockResults = [
    {
      ...
      snippet:
        "Learn how to integrate AI into your content workflow to boost efficiency and engagement.",
    ...
    },
    {
      ...
      snippet:
        "Our latest product update includes a powerful semantic search integration.",
  ...
  return mockResults.filter((result) => {
    ...
    const matchesLocale =
      filters.locales.length === 0 || filters.locales.includes(result.locale);
